...
int main()
{
	...
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			...
			if (maze[y][x] == HALL) {
				SetConsoleTextAttribute(h, BLACK); // черные
				cout << " ";
			}
			else if (maze[y][x] == WALL) {
				SetConsoleTextAttribute(h, GRAY); // серые
				cout << "#";
			}
			else if (maze[y][x] == COIN) {
				SetConsoleTextAttribute(h, YELLOW); // ярко-желтый
				cout << ".";
			}
				...
				SetConsoleTextAttribute(h, GREEN); // зеленый
				cout << "+";
			}
			else {
				cout << maze[y][x];
